...
export async function createVacancy({
    ...
    try {
        const response = await fetch(
            "http://localhost:3000/api/v1/vacancies/",
            {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
...
export async function getEmployeesFromVacancy({
    vacancy_id,
    filter,
    count = 30,
}) {
    try {
        const response = await fetch(
            `http://localhost:3000/api/v1/vacancies/${vacancy_id}`,
            {
                method: "POST",
                headers: {
                    "Content-Type": "application/json",
                },
                body: JSON.stringify({ vacancy_id, filter, count }),
                credentials: "include",
            }
        );
